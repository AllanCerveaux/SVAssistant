...
public class Encryption
{
	...
	/// </summary>
	private static int size => int.Parse(Environment.GetEnvironmentVariable("PASSWORD_SALT_SIZE") ?? "16");

	/// <summary>
	/// Gets the number of iterations for the PBKDF2 password hashing process.
	/// </summary>
	private static int itteration => int.Parse(Environment.GetEnvironmentVariable("PBKDF2_ITERATIONS") ?? "10000");

	/// <summary>
	/// Gets the hash size for the output of the password hashing process.
	/// </summary>
	private static int hashSize => int.Parse(Environment.GetEnvironmentVariable("HASH_SIZE") ?? "20");
